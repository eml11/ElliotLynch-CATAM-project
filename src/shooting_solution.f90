...
program stellarstructure
  ...
  yinner(2) = pressurec/PSUN 
  youter(2) = masst/(radiusstar**2) !((2.0*G)/(3*0.034))*(MSUN/(RSUN**2))*(masst/(radiusstar**2)) !from eddington approx
  ...
  function question4rkfunc (x,y)
    ...
    question4rkfunc(1) = (1/(4*PI))*(1/(y(1)**2))*(1/DENSUN)*((y(2))**(-1/ratiospecificheat)) !(1/(4.0*PI))*(1/(y(1)**2))*(1/densityc)*((y(2))**(-1/ratiospecificheat))
    question4rkfunc(2) = -((3.0*opacity)/(8*PI))*(MSUN/(RSUN**2))*(x/(y(1)**4))  !-(G/(4.0*PI))*(x/(y(1)**4))*((MSUN**2)/(pressurec*RSUN**4))
